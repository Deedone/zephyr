...
static void optee_get_revision(const struct device *dev)
{
	...
	data->smc_call(OPTEE_SMC_CALL_GET_OS_REVISION, 0, 0, 0, 0, 0, 0, 0,
		  &res.smccc);

	if (res.result.build_id) {
		LOG_INF("OPTEE revision %lu.%lu (%08lx)", res.result.major,
			res.result.minor, res.result.build_id);
	} else {
		LOG_INF("OPTEE revision %lu.%lu", res.result.major, res.result.minor);
	}
}
...
static bool optee_exchange_caps(const struct device *dev, unsigned long *sec_caps)
{
	...
	data->smc_call(OPTEE_SMC_EXCHANGE_CAPABILITIES, a1, 0, 0, 0, 0, 0, 0,
		  &res.smccc);
